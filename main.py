...
async def welcome_message():
    ...
    channel = discord.utils.get(client.get_all_channels(), id=welcome_channel_id)

    # Ensure we have a text channel
    if not isinstance(channel, discord.TextChannel):
        raise ValueError(f"Channel {welcome_channel_id} is not a text channel")
    ...
    print("No welcome message found, creating one now")
    view = discord.ui.View()
    button = discord.ui.Button(label="Complete Onboarding")
    view.add_item(button)
    embed = discord.Embed(
        title="Welcome to the Server!", description="Here's how to get started:"
    )
    ...
    embed.add_field(
        name="Step 3:",
        value="Complete Onboarding procedure to unlock the rest of the server.",
    )
...
):  # update server nickname from DB
    nickname = f"{firstname} {lastname}"
    with sqlite3.connect("member_data.db") as conn:
        c = conn.cursor()
        c.execute(
            "UPDATE members SET nickname = ?, firstname = ?, lastname = ? WHERE user_id = ?",
            (nickname, firstname, lastname, member.id),
        )
        conn.commit()
...
async def remove_user(user):  # Remove user from DB and delete server nickname
    ...
    if user:
        ...
        role = discord.utils.get(
            user.guild.roles, name="Maker"
        )  # Remove role "Maker" from user
...
async def update_onboard(member):  # increase onboarding status by 1
    ...
    with sqlite3.connect("member_data.db") as conn:
        ...
        c.execute(
            "SELECT onboarding_status FROM members WHERE user_id = ?", (member.id,)
        )
        row = c.fetchone()
        ...
        c.execute(
            "UPDATE members SET onboarding_status = ? WHERE user_id = ?",
            (status, member.id),
        )
...
def create_onboard_modal(fields_config, guild_id):
    ...
    class DynamicOnboardModal(discord.ui.Modal, title="Onboarding"):
        def __init__(self):
            ...
            for field_config in config_to_use[:5]:
                text_input = discord.ui.TextInput(
                    ...
                    max_length=min(
                        field_config.get("max_length", 100), 1000
                    ),  # Discord limit
                )
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try:
                # Ensure interaction is from a guild and user is a Member
                ...
                member = interaction.user  # Type narrowed to Member
                ...
                try:
                    ...
                    if settings.get("auto_role", True):
                        ...
                        if onboarded_role:
                            role = discord.utils.get(
                                interaction.guild.roles, id=onboarded_role.role_id
                            )
                            if role:
                                try:
                                    await member.add_roles(role)
        ...
        async def on_error(self, _interaction: discord.Interaction, error):
...
class OnboardButtons(discord.ui.View):
    ...
    @discord.ui.button(
        label="Complete Onboarding", style=discord.ButtonStyle.green, custom_id="1"
    )
    async def onboard(
        ...
        try:
            ...
            member_record = session.exec(
                select(Member).where(
                    Member.user_id == interaction.user.id, Member.guild_id == guild_id
                )
            ).first()
    ...
    @discord.ui.button(
        label="What is Onboarding?", style=discord.ButtonStyle.blurple, custom_id="2"
    )
    async def aboutonboard(
        self, interaction: discord.Interaction, _button: discord.ui.Button
    ):
        embed = discord.Embed(
        ...
        embed.add_field(
            name="Agreement to Server Rules",
...
async def on_member_join(member):
    ...
    with sqlite3.connect("member_data.db") as conn:
        ...
        if not existing_member:  # If it's the member's first time joining
            ...
            await member.edit(
                nick=c.execute(
                    "SELECT nickname FROM members WHERE user_id = ?", (member.id,)
                ).fetchone()[0]
            )
...
@client.tree.command(
    name="remove", description="Remove user from database, and remove user's nickname"
)
